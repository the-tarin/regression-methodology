...
- Readmitted (outcome) (binary outcome)
...
Only select covariates of interest including the ID. Rename all covariates for styling.

Remove observations with missing values (NA) for the exposure, outcome or confounders. The IDs of the missing values are shown below.
...
The surgery time is originally in HH:MM format, we re-scaled to the number of hours (numeric), as this will provide a sensible interpretation for the coefficient for surgery time. For example, 07:30 would be re-scaled to 7.5 hours.
...
The intuition behind this was that there was a lack of data for higher Charlson Indices, and the scores are somwhat "arbitrary". For example a score of 6 could mean 1 moderately severe disease, or 2-3 minor diseases. It was easier to interpret Charlson Index as either having minor or severe disease.
...
- TODO: Talk about warnings, algorithm converged, 
...
```{r}
standard_error_comparison <- rbind(c(tidy(readmissions_relative_risk_model)$std.error[1], sqrt(diag(vcovHC(readmissions_relative_risk_model, type = "HC2")))[1]), c(tidy(readmissions_relative_risk_non_linear_least_squares_model)$std.error[1], sqrt(diag(vcovHC(readmissions_relative_risk_non_linear_least_squares_model, type = "HC2")))[1]), c(tidy(readmissions_relative_risk_poisson_working_model)$std.error[1], sqrt(diag(vcovHC(readmissions_relative_risk_poisson_working_model, type = "HC2")))[1]), c(tidy(readmissions_relative_gamma_working_model)$std.error[1], sqrt(diag(vcovHC(readmissions_relative_gamma_working_model, type = "HC2")))[1]))
...
colnames(standard_error_comparison) <- c("Model Standard Errors", "Sandwich Estimator Standard Errors")
standard_error_comparison
```

We have (slight) underestimates for our standard error (and variance) for the Relative Risk Model using model standard error, there is a slight increase of standard error when we use the sandwich estimator. The standard error from the Poisson Working Model (model and sandwich estimator) have the smallest estimates.

The Non-Linear Least Squares Working Model has the highest change in standard error between model calculation and sandwich estimator calculations.

Gamma Model seems to have the smallest change in standard error between model calculations and sandwich estimator calculations.
...
- Warnings are a process of the optimisation algorithm, not an issue. 
