...
knitr::include_graphics("data/cycle_count_causal_DAG.jpeg")
```
...
```{r warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
auckland_cycle_sum_cycle_date <- auckland_cycle %>%
  group_by(date) %>%
  summarize(total_cycle_count = sum(cycle_count, na.rm = TRUE))

date_breaks <- seq(from = as.Date("2015-01-01"), to = as.Date("2022-12-01"), by = "1 year")

ggplot(auckland_cycle_sum_cycle_date, aes(x = date, y = total_cycle_count)) +
  geom_bar(stat = "identity", fill = "skyblue", color = "black") +  # Use stat = "identity" for bar heights based on data
  labs(title = "Total Cycle Count vs Date",
       x = "Date",
       y = "Total Cycle Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 0, hjust = 0, vjust = 0.5)) +  # Adjust angle for better readability
  scale_y_continuous(labels = function(x) format(x, big.mark = ",", scientific = FALSE)) +
...
```{r warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
auckland_cycle_average_covid_status <- auckland_cycle %>%
  group_by(covid_status) %>%
  summarize(avg_cycle_count = mean(cycle_count, na.rm = TRUE))
...
desired_order <- c(
  ...
  "level-1-lockdown",
  "inter-lockdown",
  "traffic-red-lockdown",
  "traffic-orange-lockdown",
  "post-covid"
)

ggplot(auckland_cycle_average_covid_status, aes(x = covid_status, y = avg_cycle_count)) +
  geom_bar(stat = "identity") +
  labs(title = "Average Cycle Count vs Covid Status",
       x = "Covid Status",
       y = "Average Cycle Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
ggplot(auckland_cycle, aes(x = year, y = weighted_average_rainfall)) +
  geom_jitter(width = 0.4, height = 0, alpha = 0.6, color = "lightblue") +
...
```{r warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
season_colors <- c("Summer" = "red", "Autumn" = "#FFD700", "Winter" = "#1E90FF", "Spring" = "#32CD32")

auckland_cycle_seasons <- auckland_cycle %>%
...
ggplot(auckland_cycle_seasons, aes(x = month, y = weighted_average_rainfall, color = season)) +
...
ggplot(auckland_cycle, aes(x = cycle_recording_site, y = weighted_average_rainfall)) +
  geom_jitter(width = 0.4, height = 0, alpha = 0.6, color = "lightblue") +
  labs(title = "Weighted Average Rainfall vs Cycle Recording Site",
       x = "Cycle Recording Site",
       y = "Weighted Average Rainfall (mm)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
```{r warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
ggplot(auckland_cycle, aes(x = weighted_average_rainfall, y = cycle_count)) +
  geom_point(alpha = 0.6, color = "grey") +
  labs(title = "Cycle Count vs Weighted Average Rainfall",
       x = "Weighted Average Rainfall (mm)",
       y = "Cycle Count") +
  theme_minimal()
```

```{r fig.width=15,fig.height=15, warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
ggplot(auckland_cycle, aes(x = weighted_average_rainfall, y = cycle_count)) +
  geom_point(alpha = 0.6, color = "grey") +
  labs(title = "Cycle Count vs Weighted Average Rainfall by Cycle Recording Site",
       x = "Weighted Average Rainfall (mm)",
       y = "Cycle Count") +
  theme(
  ...
  facet_wrap(~ cycle_recording_site, ncol = 4, scales = "free_y") +
  theme_minimal()
```
...
```{r results='hide', warning=FALSE, message=FALSE, error=FALSE, echo=FALSE}
model_summary <- summary(cycle_count_poisson_model)
pattern <- "day_of_week|month|cycle|covid"
filtered_coefficients <- model_summary$coefficients[!grepl(pattern, rownames(model_summary$coefficients)), ]
cycle_count_poisson_model_summary <- model_summary
cycle_count_poisson_model_summary$coefficients <- filtered_coefficients
```
...
```{r warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
predicted_count <- predict(cycle_count_poisson_model, newdata = auckland_cycle, type = "response")

ggplot(auckland_cycle, aes(x = weighted_average_rainfall, y = cycle_count)) +
  geom_point(alpha = 0.6, color = "grey") +
  geom_line(aes(y = predicted_count, color = "Fitted Line"), size = 1) +
  labs(title = "Cycle Count vs Weighted Average Rainfall with Fit",
       x = "Weighted Average Rainfall (mm)",
       y = "Cycle Count") +
  scale_color_manual(name = "Legend", values = c("Fitted Line" = "blue")) +
  theme_minimal()
```
...
ggplot(auckland_cycle, aes(x = weighted_average_rainfall, y = cycle_count)) +
  geom_point(alpha = 0.6, color = "grey", aes(color = "Observed")) +
  geom_line(aes(y = predicted_count, color = "Fitted"), size = 1) +
  labs(title = "Cycle Count vs Weighted Average Rainfall by Cycle Recording Site with Fit",
       x = "Weighted Average Rainfall (mm)",
       y = "Cycle Count",
       color = "Legend") +
  theme(
  ...
  facet_wrap(~ cycle_recording_site, ncol = 4, scales = "free_y") +
  theme_minimal() +
  scale_color_manual(values = c("Observed" = "grey", "Fitted" = "blue"))
```
...
```{r warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
plot(cycle_count_poisson_model, 1)
```
...
```{r results='hide', warning=FALSE, message=FALSE, error=FALSE, echo=FALSE}
model_summary <- summary(cycle_count_negbin_model)
pattern <- "day_of_week|month|cycle|covid"
filtered_coefficients <- model_summary$coefficients[!grepl(pattern, rownames(model_summary$coefficients)), ]
cycle_count_negbin_model_summary <- model_summary
cycle_count_negbin_model_summary$coefficients <- filtered_coefficients
```
...
```{r warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
predicted_cycle <- predict(cycle_count_negbin_model, newdata = auckland_cycle, type = "response")

ggplot(auckland_cycle, aes(x = weighted_average_rainfall, y = cycle_count)) +
  geom_point(alpha = 0.6, color = "grey") +
  geom_line(aes(y = predicted_cycle), color = "blue", size = 1) +
  labs(title = "Cycle Count vs Weighted Average Rainfall",
       x = "Weighted Average Rainfall (mm)",
       y = "Cycle Count") +
  theme_minimal()
...
```{r fig.width=15,fig.height=15, warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
predicted_count <- predict(cycle_count_negbin_model, newdata = auckland_cycle, type = "response")
...
```{r warning=FALSE, message=FALSE, error=FALSE, echo=FALSE, class.output='centered-plot'}
plot(cycle_count_negbin_model, 1)
```
